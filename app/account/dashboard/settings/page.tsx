...
export default function Settings() {
  ...
  const [owncloudError, setOwncloudError] = useState('')
  const [deleteSteps, setDeleteSteps] = useState({
    owncloud: { status: 'pending', message: 'Pending' },
  ...
  const deleteAccount = async () => {
    ...
    try {
      // 1. Delete ownCloud account
      const owncloudResponse = await fetch('/api/users/delete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const owncloudData = await owncloudResponse.json()
      
      if (owncloudData.steps) {
        setDeleteSteps(prev => ({
          ...prev,
          owncloud: owncloudData.steps.owncloud
        }))
      }
      
      if (!owncloudResponse.ok || owncloudData.steps?.owncloud.status === 'error') {
        if (owncloudData.error === 'Failed to delete user from ownCloud') {
          setOwncloudError('Failed to delete your ownCloud account. Please try again or contact support.')
        } else {
          setOtpError(owncloudData.error || 'Failed to delete ownCloud account')
        }
        setIsDeleting(false)
        return
      }
            ...
            <Dialog>
              ...
              {deleteStep === 'initial' ? (
                <DialogContent>
                  ...
                  <p className="text-sm font-bold mt-2">This will remove your data from the following services:</p>
                  <ul className="list-disc list-inside text-sm">
                    <li>ownCloud</li>
                    <li>LibreCloud Internal DB</li>
                  </ul>

                  <p className="text-sm mt-2">If you need help deleting data from other services, please <a href="https://librecloud.org/account/dashboard/support" className="text-blue-500 hover:underline">contact support</a>. We will implement more self-service deletion features in the future.</p>
                  <p className="text-sm mb-4">You will be asked for a final confirmation after confirming your identity.</p>
                ...
                <DialogContent>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between items-center">
                      <span>ownCloud</span>
                      {renderStepStatus(deleteSteps.owncloud)}
                    </div>
                    <div className="flex justify-between items-center">
                      <span>Database</span>
                  ...
                  {owncloudError && (
                    <p className="text-sm text-destructive mt-2">{owncloudError}</p>
                  )}
                  ...
                  <DialogFooter className="mt-4">
                    {isDeleting ? (
                      ...
                      <Button 
                        className="cursor-pointer" 
                        variant="destructive" 
                        disabled={deleteSteps.owncloud.status === 'error' || deleteSteps.database.status === 'error'} 
